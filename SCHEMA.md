...
Every `diagnostics.ambiguousMatches` entry represents an unresolved grouping
and must be surfaced as such. Its `reason` is human-readable detail, not a
classification field. Successfully resolved assignments are recorded
separately in `diagnostics.dateResolvedMatches`.

`diagnostics.observedRounds` is an objective list of the numbered `R<n>`
rounds present in the data. Gaps are not classified as errors because they can
represent byes, forfeits, or intentionally unscheduled rounds.
...
## File 3: timeout_history.json

Generated by `scripts/fetch_league_data.py`. This durable ledger records
timeouts first observed by successful tracker fetches. It is distinct from
`leagueData.json`, which remains the source of aggregate match statistics.
...
```typescript
{
  ...
  observedCounts: {
    [matchUrl: string]: {
      [username: string]: number    // Highest observed timeout count
    }
  },
  events: {
    id: string,                     // `${matchUrl}|${username}|${ordinal}`
    matchUrl: string,
    username: string,               // Lowercase
    ordinal: number,
    detectedAt: string              // ISO 8601 timestamp (UTC)
  }[]
}
```

On its first creation, the ledger baselines existing counts without events.
Later runs append an event for each count increase and never lower stored
counts. `detectedAt` means when the tracker first saw the timeout, not the
exact time the game ended; it is the only date used for the dashboard's recent
timeout window.

---

## File 4: timeoutData.json
