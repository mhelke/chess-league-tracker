...
Generated by `scripts/fetch_player_ratings.py`. This is a compact current-rating
lookup, not rating history. It contains only players who both appear in this
site's league history and are present in the latest Chess.com club-members
response.
...
Recruitment must use an entry only when `membershipStatus` is `"verified"`
and both membership and rating timestamps are fresh. A member who leaves the
club is removed during the next successful nightly membership validation; their
historical league records remain unchanged.
